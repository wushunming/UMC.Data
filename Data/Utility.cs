...
namespace UMC.Data
{
    ...
    public class Utility
    {
        ...
        public static int Scanning(string query, string msg)
        {
            ...
            var g = new Guid(md).GetHashCode();

            var storeEntity = Data.Database.Instance().ObjectEntity<Data.Entities.Click>();
            storeEntity.Where.And().Equal(new Data.Entities.Click { Code = g });
            ...
            storeEntity.IFF(e => e.Update(c) == 0, e => e.Insert(c));
        ...
        public static int Scanning(int Key, string query, string msg)
        {
            ...
            var storeEntity = Data.Database.Instance().ObjectEntity<Data.Entities.Click>();
            storeEntity.Where.And().Equal(new Data.Entities.Click { Code = Key });
            var c = new Data.Entities.Click
            {
                Query = query,
                Code = Key,
        ...
        public static string Scanning(int code)
        {
            ...
            var storeEntity = Data.Database.Instance().ObjectEntity<Data.Entities.Click>();
            storeEntity.Where.And().Equal(new Data.Entities.Click { Code = code });
            var scanning = storeEntity.Single();
            if (scanning != null)
            {
                cstr = scanning.Query;
        ...
        {
            var storeEntity = Data.Database.Instance().ObjectEntity<Data.Entities.Click>();
            var scan = storeEntity.Where.And().In(new Data.Entities.Click
            {
                Code = UMC.Data.Utility.Guid(code, true).Value.GetHashCode(),
            }, UMC.Data.Utility.Parse62Decode(code)).Entities.Single();
            hashtable = null;
            if (scan != null)
            {
                if (scan.Quality == -1)
                ...
                else
                {
                    ...
                    {
                        storeEntity.Update("{0}+{1}", new Data.Entities.Click { Quality = 1 });
                    }
                    else
                    {
                        storeEntity.Update(new Data.Entities.Click { Quality = 1 });
                    }
                }
